...
.droptimebox+.bootstrap-datetimepicker-widget {
...
<script type="text/javascript">
						...
						$("#member")
								.change(
										function() {
											...
											var val = $("#member").val();
											if (val > 0) {
												var html = '<tr><th width="22%">Denomination</th><th width="22%">Discrepancy Type</th><th width="22%">Mutilated Type</th><th width="22%">Note Serial Number</th><th>No. of Notes</th><th>Value</th><th>Print Year</th><th>Date on Shrink Wrap</th><th>Time of Detection</th><th>Remarks</th></tr>';
												for (i = 0; i < val; i++) {
													html += '<tr id="main'+i+'">'
															+ '<td width="22%"><input type="text" id="Denomination'
															+ i
															+ '" class="form-control input-margin" name="Denomination" onkeyup="doAjaxPost('
															+ i
															+ ')" value="" autofocus></td>'
															+ '<td width="25%"><select name="discrepancyType" id="discrepancyType'
															+ i
															+ '" class="form-control input-margin" onchange="getMutilValue('
															+ i
															+ '); getFakeNotePrintYear('
															+ i
															+ ');getSerialNumber('
															+ i
															+ ')"><option value="SHORTAGE">SHORTAGE</option><option value="EXCESS">EXCESS</option><option value="FAKE">FAKE</option><option value="MUTILATED">MUTILATED</option></select></td>'
															+ '<td width="22%"><div class="desc" id="mutil'+i+'" ><select name="mutilType'
															+ i
															+ '" disabled="disabled" id="mutilType'
															+ i
															+ '" class="form-control input-margin"  onchange="doAjaxPost('
															+ i
															+ ')"><option value="">Select</option><option value="FULL VALUE">FULL VALUE</option><option value="HALF VALUE">HALF VALUE</option><option value="ZERO VALUE">ZERO VALUE</option></td></div>'
															...
															+ '<td width="22%"><input type="text" id="NumberOfNotes'
															+ i
															+ '"  class="form-control input-margin" name="numberOfNotes" value="" onkeyup="doAjaxPost('
															+ i
															+ ')" ></td>'
												...
												$('#table1')
														.append(
																'<tr id=addmoreButton'
																		...
																		+ (i - 1)
																		+ ')">Replicate Value</button></td><td><button type="button"  class="addmoreButton" onclick="addRow('
																		+ (i - 1)
																		+ ')">Add Blank Row</button></td><td></td><td></td></tr>');
											} else {
												$('#table1').empty();
											}
										});
	...
	function replicateValue(i) {
		...
		var data = '<tr id="main'+countrow+'">'
				...
				+ '); getFakeNotePrintYear('
				+ countrow
				+ ')"><option value="'+discrepancyType+'">'
				+ discrepancyType
				+ '</option></select></td>'
				+ '<td width="22%"><div class="desc" id="mutil'+countrow+'" ><select name="mutilType'
				+ countrow
				+ '"  id="mutilType'
				+ countrow
				+ '" class="form-control input-margin" onchange="doAjaxPost('
				+ countrow
				+ ')"><option value="'+mutilType+'">'
				+ mutilType
				+ '</option></select></td></div>'
				+ '<td width="22%"><input type="text" id="NoteSerialNumber'+countrow+'" class="form-control input-margin" name="NoteSerialNumber" value=""></td>'
				+ '<td width="22%"><input type="text" id="NumberOfNotes'
				+ countrow
				+ '"  class="form-control input-margin" name="numberOfNotes" value="'
				+ numberOfNotes
				+ '" onkeyup="doAjaxPost('
				+ countrow
				+ ')" ></td>'
				...
				+ '<td width="22%"><input type="text" id="dateOnShrinkWrap'
				+ countrow
				+ '"  class="form-control input-margin" name="dateOnShrinkWrap" value="'
				+ dateOnShrinkWrap
				+ '" onmouseover="doAjaxForDate('
				+ countrow
				+ ')" ></td>'
				+ '<td width="22%"><input type="text" id="timeOfShrinkWrap'
				+ countrow
				+ '"  class="form-control input-margin droptimebox" name="timeOfShrinkWrap" value="'
				+ timeOfShrinkWrap
				+ '" onmouseover="doAjaxForDate('
				+ countrow
				+ ')" ></td>'
	...
	function doAjaxPostInsert(str) {
		...
		for (i = 0; i <= countrow; i++) {
			...
			if (($('#Denomination' + i).val() != undefined)
					&& $('#Denomination' + i).val() != 2000
					&& $('#Denomination' + i).val() != 1000
					&& $('#Denomination' + i).val() != 500
					&& $('#Denomination' + i).val() != 200
					&& $('#Denomination' + i).val() != 100
					&& $('#Denomination' + i).val() != 50
					&& $('#Denomination' + i).val() != 20
					&& $('#Denomination' + i).val() != 10
					&& $('#Denomination' + i).val() != 5
					&& $('#Denomination' + i).val() != 2
					&& $('#Denomination' + i).val() != 1) {
				$('#err8').show();
				isValid = false;
			}
			if (($('#discrepancyType' + i).val() != undefined)
					&& $('#discrepancyType' + i).val() == "") {
				$('#err9').show();
				isValid = false;
			}
			...
			if (($('#NumberOfNotes' + i).val() != undefined)
					&& $('#NumberOfNotes' + i).val() == "") {
				$('#err12').show();
				isValid = false;
			}
			...
			if (($('#printYear' + i).val() != undefined)
					&& isNaN($('#printYear' + i).val())) {
				$('#err15').show();
				isValid = false;
			}
			...
			if (($('#Denomination' + i).val() != undefined) && isValid) {
				discrepancyAllocations.push({
					"denomination" : $('#Denomination' + i).val(),
					"discrepancyType" : $('#discrepancyType' + i).val(),
					"mutilType" : $('#mutilType' + i).val(),
					"noteSerialNumber" : $('#NoteSerialNumber' + i).val(),
					"numberOfNotes" : $('#NumberOfNotes' + i).val(),
					"value" : $('#Value' + i).val(),
									...
									<form:form action="#" method="post" modelAttribute="user"
										...
										<div class="col-lg-6 form-group">
											...
											<form:input path="srNo" id="srNo" name="srNo"
												cssClass="form-control" />
											<label id="errSrNo" style="display: none; color: red">Please
												Enter Sr Number</label>
										</div>
										...
										<div class="col-lg-6 form-group">
											...
											<label id="err7" style="display: none; color: red">Please
												Enter Account Number</label> <label id="err17"
										...
										<div class="form-group">
											...
											<div id="container">
												...
												<label id="err8" style="display: none; color: red">Please
													...
													style="display: none; color: red">Please Enter
													Valid Print Year</label> <label id="err16"
													style="display: none; color: red">Please Enter
													Mutilated type</label>
											</div>
										</div>
										<div class="col-lg-12">
