...
html += '<tr id="main'+i+'">'+
		...
		'<td><button type="button" onclick="deleteRow('+i+');doAjaxForTotal()">-</button></td>'+ 
...
function replicateValue(i){
	...
	'<td><input type="text" id="Denomination'+countrow+'" maxlength="4" minlength="1"  class="form-control input-margin" name="Denomination" value="'+denomData+'"  onkeyup="doAjaxPost('+countrow+')" autofocus="autofocus" ></td>'+
	...
	'<td><button type="button" onclick="deleteRow('+countrow+');doAjaxForTotal()">-</button></td>'+
...
function addRow(i){
	...
	i++;
	var data = '<tr id="main'+countrow+'">'+
	'<td><input type="text" id="Denomination'+countrow+'" maxlength="4" minlength="1"  class="form-control input-margin" name="Denomination" value="" onkeyup="doAjaxPost('+countrow+')"  autofocus="autofocus" ></td>'+
...
function SavePrint(str) {
	...
	var dsb={
			...
			"total":$('#Total'+str).val(),
			"processingOrVault":$('#main'+str).find('input[type=radio]:checked').val(),
								...
								<div class="col-lg-10">
									...
									<form:form id="" action="" method="post" modelAttribute="user" autocomplete="off">
										...
										<div align="right">
											...
											<label id="err3" style="display: none;color: red">Enter Valid Bundle</label>
