...
<body oncontextmenu="return false;">
	<div id="wrapper">
		...
		<c:set var="currentDate" scope="session"><fmt:formatDate value="${now}" pattern="yyyy-MM-dd"/></c:set>
		
		<c:set var="binDate" scope="session"><fmt:formatDate value="${binTransactionDate}" pattern="yyyy-MM-dd"/></c:set>
		...
		<c:if test="${binTransactioEODDate !=NULL || binTransactionDate == NULL}"><jsp:include page="common.jsp" /></c:if>
		
		<c:if test="${binTransactioEODDate == NULL and binTransactionDate != NULL}">
	       <c:if test="${currentDate  eq binDate}"><jsp:include page="common.jsp" /></c:if>
	       <c:if test="${currentDate  ne binDate}"><div><jsp:include page="logOut.jsp" /></div></c:if>
		</c:if>
		<div id="page-wrapper">
			<!-- /.row -->
			<div class="row">
				<div class="col-lg-12">
					...
					<h1 class="page-header">Welcome to <!-- e@ICMC --> Vault Management System</h1>
						...
						<c:if test="${binTransactioEODDate == NULL and binTransactionDate != NULL and (currentDate  ne binDate)}">
