		...
		var id = $('input[name=solId]:checked').val()
				$.ajax({
					...
					success : function(response) {
						...
						if(response[0].category=="UNPROCESS"){
							for(var i=0; i < response.length; i++){
								trHTML += '<tr><input type="hidden" value="'+response[i].id+'" id="id"><td id="category'+(i+1)+'">'
								+ response[i].category
								+ '</td><td width="10%"></td><td id="denomination'+(i+1)+'">'
								+ response[i].denomination
								+ '</td><td width="10%"></td><td>'
								+ response[i].bundle
								+ '</td><td width="10%"></td><td><input type=text  readonly="true"  id="qr'+(i+1)+'"></td><td><span class="tick-msg"></td></tr>';
							}
						}else{
						var count = 0;
						$
