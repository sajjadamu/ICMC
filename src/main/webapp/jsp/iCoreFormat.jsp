					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<div class="region-con">
								<form:form id="userPage" name="userPage" action="iCoreFormat"
										...
										<table class="table table-striped table-bordered table-hover"
											...
											<tbody>
												...
												<tr>
													...
													<td>C</td>
													<td>${sairremTotal+samutcurTotal}SHORT/DISCCASH</td>
												</tr>
												<tr>
															...
															<tr>
																...
																<c:choose>
																	<c:when test="${row.accountTellerCam == 'ACCOUNT'}">
																		...
																		<td>INR<fmt:formatNumber pattern="0000"
																				value="${row.accountNumber.substring(0,4)}" /></td>
																		...
																		<c:choose>
																			...
																			<%-- <fmt:formatDate pattern="dd.MM.yyyy" value="${row.discrepancyDate}" /> --%>
																		</c:choose>
																		<c:choose>
																			<c:when
																		...
																		<c:choose>
																			<c:when
																				...
																				<td>${innerRow.value}.00${innerRow.accountNumber}/Shortage</td>
																			</c:when>
																		</c:choose>
																		<c:choose>
																			<c:when
																				test="${innerRow.discrepancyType == 'EXCESS'}">
																				<td>${innerRow.value}.00${innerRow.accountNumber}/Excess</td>
																			</c:when>
																		</c:choose>
																...
																<c:choose>
																	<c:when
																		test="${row.accountTellerCam == 'TELLER' && (innerRow.denomination==2000 || innerRow.denomination==500 || innerRow.denomination==200 ||  innerRow.denomination==100)}">

																		<c:choose>
																			<c:when test="${innerRow.discrepancyType=='EXCESS'}">
																				<td><fmt:formatNumber pattern="0000"
																						value="${row.solId}" />SLDISCUR</td>
																				<td>INR<fmt:formatNumber pattern="0000"
																						value="${row.solId}" /></td>
																				<td>C</td>
																			</c:when>
																		...
																		<c:choose>
																			<c:when test="${innerRow.discrepancyType == 'FAKE'}">
																				<td>
																					${innerRow.value}.00${innerRow.customerName}/Counterfeit</td>
																			</c:when>
																		</c:choose>
																		...
																		<c:choose>
																			<c:when
																				test="${innerRow.discrepancyType == 'SHORTAGE'}">
																				<td>${innerRow.value}.00${innerRow.customerName}/Shortage</td>
																			</c:when>
																		</c:choose>
																		...
																		<c:choose>
																			<c:when
																				test="${innerRow.discrepancyType == 'EXCESS'}">
																				<td>${innerRow.value}.00${innerRow.customerName}/Excess</td>
																			</c:when>
																		</c:choose>
																...
																<c:choose>
																	<c:when
																		test="${row.accountTellerCam == 'TELLER' && (innerRow.denomination==50 || innerRow.denomination==20 || innerRow.denomination==10 ||  innerRow.denomination==5
												...
												<c:forEach var="row" items="${discrepancyList}">
													<c:forEach var="innerRow"
														...
														|| innerRow.denomination==2 || innerRow.denomination==1)}">
															...
															<!-- end by shahabuddin -->
															<c:choose>
																<c:when test="${row.accountTellerCam == 'DSB'}">
																	<fmt:formatDate value="${innerRow.discrepancyDate}"
																		var="parsedEmpDate" pattern="dd-MM" />
																	<c:if test="${innerRow.discrepancyType=='EXCESS'}">
																		<tr>
																			<td><fmt:formatNumber pattern="0000"
																					value="0104" />IBTRANCR</td>
																			<td>INR<fmt:formatNumber pattern="0000"
																					value="${row.solId}" /></td>
																			<td>C</td>
																			<td>${innerRow.value}.00/${row.srNo}/${innerRow.customerName}/${icmcName}/${parsedEmpDate}</td>
																		</tr>
																	</c:if>
																	<c:if test="${innerRow.discrepancyType !='EXCESS'}">
																		<tr>
																			<td><fmt:formatNumber pattern="0000"
																					value="0104" />SADISCUR</td>
																			<td>INR<fmt:formatNumber pattern="0000"
																					value="${row.solId}" /></td>
																			<td>D</td>
																			<td>${innerRow.value}.00/${row.srNo}/${innerRow.customerName}/${icmcName}/${parsedEmpDate}</td>
