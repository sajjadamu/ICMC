 ...
 *
 */
public final class ADLoginAuthenticationProvider extends AbstractLdapAuthenticationProvider {
	 ...
	 */
	public ADLoginAuthenticationProvider(String domain, String url, String rootDn) {
	...
	protected DirContextOperations doAuthentication(UsernamePasswordAuthenticationToken auth) {
		...
		if (userAdministrationService.isValidUser(username) == null) {
			logger.error("User is not Registered in Application: " + username);
		...
		/*
		 * DirContext ctx = null; try { ctx = getDirContext(); logger.info(
		 * "searchForUser {}: ", username); DirContextOperations
		 * dirContextOperations = searchForUser(ctx, username); logger.info(
		 * "searchForUser {}, dirContextOperations {}: ", username,
		 * dirContextOperations); return dirContextOperations; } catch
		 * (Exception e) { logger.error(
		 * "Failed to locate directory entry for authenticated user: " +
		 * username, e); throw badCredentials(e); } finally {
	...
	protected Collection<? extends GrantedAuthority> loadUserAuthorities(DirContextOperations userData, String username,
		 ...
		 * logger.info(
		 * "No User or Authorities found in icmc database for username "
		 * +username); return AuthorityUtils.NO_AUTHORITIES; }
		 * 
		 * logger.info(
		 * "User Authorities found in icmc database for username:{}, Authorities:{}: "
	...
	private DirContextOperations searchForUser(DirContext context, String username) throws NamingException {
		...
		final NamingEnumeration<SearchResult> resultsEnum = context.search("DC=" + this.domain + ",DC=com",
				s1.toString(), searchcontrols);
		 ...
		 * searchControls, searchRoot, searchFilter.toString(), new Object[] {
		 * bindPrincipal }); } catch (IncorrectResultSizeDataAccessException
		 * incorrectResults) { // Search should never return multiple results if
		 * properly configured - just // rethrow if
		 * (incorrectResults.getActualSize() != 0) { throw incorrectResults; }
		 * // If we found no results, then the username/password did not match
		 * UsernameNotFoundException userNameNotFoundException = new
		 * UsernameNotFoundException( "User " + username +
		 * " not found in directory.", incorrectResults); throw
		 * badCredentials(userNameNotFoundException); }
		 */
	}
	...
	public boolean checkUserCredentials(String s, String s1, String dom) throws Exception { // s
																							// -
																							// Username,s1-Password
		...
		try {
			...
			logger.info(
					"Indval::::::::::::::::" + INDVAL + ":::::ind::::::::::" + ind + "flag:::::::::::::::::::" + flag);
	...
	private Hashtable getDN(String s, String dom) throws Exception {
		...
		s1.append(s);
		s1.append(")( distinguishedName=*))");
		NamingEnumeration namingenumeration = dircontext.search("DC=" + dom + ",DC=com", s1.toString(), searchcontrols);
