...
function doAjaxPostUnlockUser() {  
	...
	// get the form values  
	  ...
	  $.ajax({  
	    type: "POST", 
	    url: "././unlockLockedUser",
	    data: "id="+id,
	    success: function(response){  
	    	alert("Selected user has been unlocked")
	    	window.location='././viewLockedUser';
	    }, 
	    error: function(e){  
	      alert(' Error: ' + e);  
	    }  
...
<body oncontextmenu="return false;" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-237" data-genuitec-path="/Currency/src/main/webapp/jsp/viewLockedUser.jsp">
    <div id="wrapper" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-237" data-genuitec-path="/Currency/src/main/webapp/jsp/viewLockedUser.jsp">
        <!-- Navigation -->
                                ...
                                <table class="table table-striped table-bordered table-hover" id="tableValue">
                                    ...
                                    <tbody>
            							<c:forEach var="row" items="${records}">
											<tr id="xy${row.id}">
												<td id="userId">${row.id}</td>
												<td>${row.name}</td>
												<td><fmt:formatDate pattern="dd-MMM-yy HH:mm:ss.SSS"  value="${row.lastAuthFail.time}" /></td>
					 							<td><input type="button" value="Unlock" onclick="doAjaxPostUnlockUser()">
					 							</td>
											</tr>
										</c:forEach>
                                    </tbody>
