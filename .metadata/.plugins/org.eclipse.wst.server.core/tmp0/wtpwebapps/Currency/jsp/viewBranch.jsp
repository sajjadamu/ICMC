...
<head>
    ...
    <title>ICICI : Branch Records</title>
...
<body oncontextmenu="return false;" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-211" data-genuitec-path="/Currency/src/main/webapp/jsp/viewBranch.jsp">
    <div id="wrapper" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-211" data-genuitec-path="/Currency/src/main/webapp/jsp/viewBranch.jsp">
        <!-- Navigation -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            <div align="center" style="color:white; background:green;"><b>${branchRemovalSuccessMsg}</b></div><br>
                                ...
                                <table class="table table-striped table-bordered table-hover"  id="tableValue">
                                    <thead>
                                       <tr>
                                       		<th>Sol ID</th>
											<th>Branch</th>
											<th>Servicing ICMC (SI)</th>
											<th>Jurisdiction ICMC (JI)</th>
											<th>RBI Name</th>
											<th>Zone</th>
											<th>Region</th>
											<th>Address</th>
                                    ...
                                    <tbody>
                                      <c:forEach var="row" items="${records}">
										<tr>
											<td>${row.solId}</td>
											<td>${row.branch}</td>
											<td>${row.servicingICMC}</td>
											<td>${row.jurisdictionICMC}</td>
											<td>${row.rbiName}</td>
											<td>${row.zone}</td>
											<td>${row.region}</td>
											<td>${row.address}</td>
											...
											<td>${row.pincode}</td>
											<td>${row.status}</td>
											<td><sec:authorize access="hasRole('UPDATE_BRANCH')">
													<a href="editBranch?id=${row.id}">Edit</a>
												</sec:authorize>
											</td>
											<td><sec:authorize access="hasRole('UPDATE_BRANCH')">
													<a href="removeBranch?id=${row.id}">Remove</a>
												</sec:authorize>
											</td>
										</tr>
									  </c:forEach>
