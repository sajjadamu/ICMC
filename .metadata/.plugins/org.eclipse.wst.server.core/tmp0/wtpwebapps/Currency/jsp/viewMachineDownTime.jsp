...
<body oncontextmenu="return false;" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-240" data-genuitec-path="/Currency/src/main/webapp/jsp/viewMachineDownTime.jsp">
    <div id="wrapper" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-240" data-genuitec-path="/Currency/src/main/webapp/jsp/viewMachineDownTime.jsp">
        <!-- Navigation -->
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
	                        ...
	                        <ul>
	                        	<li>
	                        		<sec:authorize access="hasRole('ADD_MACHINE_DOWNTIME_UPDATION')">
	                        			<a href="././MachineDowntimeUpdation"><i class="fa fa-table fa-fw"></i> Add Machine Downtime</a>
	                        		</sec:authorize>
	                        	</li>
							</ul>Machine Downtime Details
                        </div>
				...
				<tr><% i++;%>
					<%-- <td><%=i %></td> --%>
					<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss"  value="${row.machineDownDateFrom}" /></td>
					<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss"  value="${row.machineDownDateTo}" /></td>
					<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss"  value="${row.engineerAttendedCall}" /></td>
					<td>${row.machineType}</td>
					<td>${row.machineNo}</td>
					<%-- <td><fmt:formatDate pattern="yyyy-MM-dd"  value="${row.timeTo}" /></td>
					<td><fmt:formatDate pattern="yyyy-MM-dd"  value="${row.timeFrom}" /></td>--%>
					<td>${row.downtimeReason}</td>
					<td>${row.remarks}</td>
					<td><sec:authorize access="hasRole('UPDATE_MACHINE_DOWNTIME_UPDATION')">
							<a href="editMachineDownTime?id=${row.id}">Edit</a>
						</sec:authorize></td>
				</tr>
			</c:forEach><%i=0; %>
                                    </tbody> 
