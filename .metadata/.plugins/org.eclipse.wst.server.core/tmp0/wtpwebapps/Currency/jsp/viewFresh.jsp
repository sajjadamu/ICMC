...
<body oncontextmenu="return false;" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-227" data-genuitec-path="/Currency/src/main/webapp/jsp/viewFresh.jsp">
	<div id="wrapper" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-227" data-genuitec-path="/Currency/src/main/webapp/jsp/viewFresh.jsp">
		<!-- Navigation -->
							...
							<ul>
								<li>
									<sec:authorize access="hasRole('ADD_FRESH_FROM_RBI')">
										<a href="././freshFromRbi"><i class="fa fa-table fa-fw"></i> Add Fresh Data</a>
									</sec:authorize>
								</li>
							</ul>Fresh Data List
						</div>
								...
								<form id="showAll">
									...
									<table class="table table-striped table-bordered table-hover"  id="tableValue">
										...
										<tbody>
											...
											<c:choose>
    <c:when test="${row.cashType=='NOTES'}">
       	<td><sec:authorize access="hasRole('UPDATE_FRESH_FROM_RBI')">
												<a href="editFreshRBI?id=${row.id}">Edit</a></sec:authorize>
											</td>
       
    </c:when>    
    <c:otherwise>
       	<td><sec:authorize access="hasRole('UPDATE_FRESH_FROM_RBI')">
												<a href="editCoFreshRBI?id=${row.id}">Edit</a></sec:authorize>
											</td>
												...
												<a href="editFreshRBI?id=${row.id}">Edit</a></sec:authorize>
													...
													<td><a href="rePrintFreshQR?id=${row.id}">Re-print QR</a></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
	    	...
	    	$('#tableValue').dataTable({
    		 ...
    		 drawCallback: function () {
	    	        ...
	    	        $( api.table().footer() ).html('<th colspan="4" style="text-align:right">Current Page Total: '+
	    	        		api.column( 2, {page:'current'} ).data().sum().toLocaleString('en-IN')+'; All Pages Total: '+
	    	        		api.column( 2 ).data().sum().toLocaleString('en-IN')+'</th>'
	    	        );
	    	      }
	    	});
	    });
